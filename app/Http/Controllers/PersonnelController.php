...
{   
    ...
    public function index(){
        ...
        return view('officer.personnel.home.index', compact('officer', 'position'));
    }
    ...
    public function transaction(){
        ...
        $position = position::all()->where('relation_id',Auth::user()->id);
        return view('officer.personnel.transaction.index', compact('officer', 'depositor', 'uploaded_document', 'transaction', 'position'));
    }

    //redirect to view the depositor personnel
    public function view_application_depositor(Request $request){
        $officer = officer::all()->where('relation_id', Auth::user()->id);
        $data = Depositor::all()->where('depositor_id', $request->depositor_id);
        $guardians = guardian::all()->where('depositor_id', $request->depositor_id);
        $branch = Branch::all();
        $branch_under = Branch_under::all();
        $profile_pic = uploaded_document::all()->where('depositor_id', $request->depositor_id);
        $position = position::all()->where('relation_id',Auth::user()->id);
        return view('officer.personnel.transaction.view_depositor',compact('officer','profile_pic', 'data', 'guardians', 'branch', 'branch_under', 'position'));
    }
    ...
    Notification::send($user, new StatusAlert($statusData));
       ...
       //process on adding the transaction to history table
        ...
        $brachlvl =["3"];
        $branchcheck1 = Transaction::where('depositor_id', $transaction->depositor_id)->whereIn('level_id', $brachlvl)->first();

        if($branchcheck1){
            //has already branch transaction
