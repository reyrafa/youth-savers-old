...
class BranchController extends Controller
{
    ...
    public function findBranchUnder(Request $request){
      ...
      public function index(){
        ...
        return view('officer.branch.home.index', compact('officer', 'position', 'branch'));
      }
      ...
      //redirecting to importing YSC page
      public function importingExistingMember(){
        $officialMember = OfficialMember::all();
        $depositor = Depositor::all();
        $branch = Branch_under::all();
        $officer = officer::all()->where('relation_id', Auth::user()->id);
        return view('officer.branch.importing.index', compact('officer', 'officialMember', 'depositor', 'branch'));
      }
