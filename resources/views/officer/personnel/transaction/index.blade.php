...
@section('content')
    ...
    <div class="table-responsive">
        <table class="table stripe align-middle hover table-bordered" id="transaction_table">
            <thead>
                <th class="border">No</th>
                <th class="border">Fullname</th>
           
                <th class="border">Signature</th>
                <th class="border">2*2 Picture</th>
                <th class="border">Birth Certificate</th>
                        ...
                        <td><a class="btn disabled" style="background: #808080; color:white; width: 40px; height: 40px; border-radius: 5px;">{{++$count}}</a></td>
                            ...
                            @foreach($uploaded_document as $document_info)
                                @if($transaction_info->depositor_id == $document_info->depositor_id)
                                    ...
                                    <td>{{date("D F Y", strtotime($document_info->created_at))}}</td>
                                @endif
                            @endforeach
                                    <td><a href="#" 
                                    ...
                                    <a href={{"/personnel/view/depositor/".$transaction_info->depositor_id}}
    ...
    <div class="modal fade" id="approve_transaction" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true" aria-labelledby="exampleModalLabel"> 
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form  action="/personnel/approved/application" method="POST" enctype="multipart/form-data">
                @csrf
    ...
    <div class="modal fade" id="denied_transaction" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true" aria-labelledby="exampleModalLabel"> 
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form  action="/personnel/denied/application" method="POST" enctype="multipart/form-data">
                @csrf
                 <div class="modal-body">
                 ...
                 <div class="modal-footer">
                     <button data-bs-dismiss="modal" type="button" class="btn btn-secondary">Close</button>
                     <button type="submit" class="btn btn-danger" id="submit">Disapprove</button>
                 </div>
